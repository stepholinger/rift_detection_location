...
print("Loading input data for cluster for " + str(numClusters) + " clusters...")
...
for n in range(numClusters):
    waveFile = h5py.File(templatePath + str(numClusters) + "/aligned_cluster" + str(n) + "_waveform_matrix_" + str(prefiltFreq[0]) + "-" + str(prefiltFreq[1]) + "Hz.h5","r")
...
#cluster_labels = np.random.randint(0,high=numClusters,size=len(waves))
#print(min(cluster_labels))
#print(max(cluster_labels))

# test whether a horizontal line breaks the silhouette method (it does)
#X = waves[:90,:]
#X = np.vstack((X,np.random.rand(10,(snipLen*fs+1)*3)*100))
#X = np.vstack((X,np.ones((10,(snipLen*fs+1)*3),'float64')*100))
#print(X)
#cluster_labels = cluster_labels[:80]
#cluster_labels = np.append(cluster_labels,np.ones((20,1),'float64'))

# Compute the silhouette scores for each sample and save results
print("Starting silhouette value calculation for " + str(numClusters) + " clusters...")
sample_silhouette_values = silhouette_samples(X, cluster_labels.flatten(),metric='correlation')
#silFile = h5py.File(templatePath + str(numClusters)  +  "/" + str(numClusters) + "_cluster_silhoutte_" + str(prefiltFreq[0]) + "-" + str(prefiltFreq[1]) + "Hz.h5","w")
#silFile.create_dataset("scores",data=sample_silhouette_values)
#silFile.close()

# load silhouette values
#silFile = h5py.File(templatePath + str(numClusters)  +  "/" + str(numClusters) + "_cluster_silhoutte_" + str(prefiltFreq[0]) + "-" + str(prefiltFreq[1]) + "Hz.h5","r")
...
plt.suptitle(("Silhouette Analysis for Normalized K-Shape "
              "with n = %d" % numClusters),
             fontsize=14, fontweight='bold')

plt.savefig(templatePath + str(numClusters) + "/" + str(numClusters) + "_cluster_silhoutte" + ".png")
plt.show()
plt.close()
